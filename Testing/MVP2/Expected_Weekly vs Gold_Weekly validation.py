...
dbutils.widgets.text("lnd_arch_Path","/mnt/data/load_date=2023-07-17/101_Daily_Sales.705.20230606062318.csv")
rawFilePath=dbutils.widgets.get("lnd_arch_Path")
dbutils.widgets.text("Gold_weekly_Path","/mnt/gold/101_Daily_Sales.705.20230606062318.csv")
goldFilePath=dbutils.widgets.get("Gold_weekly_Path")
...
sfile1=spark.sql("select path,name from vw_rawFileList where name like '101_Daily_Sales.097.%'")
...
display(weeklydf)
weeklydf.createOrReplaceTempView('weekly_vw')
...
# MAGIC %md # Duplicate check on KCMS data
...
# MAGIC %sql
# MAGIC
# MAGIC select 'raw', DIV,STORE,ITEM_CONSUMER_UPC,POS_DATE,cast(POS_NET_DOL_AMOUNT as decimal (8,3)),cast(POS_UOM_QTY as decimal (4,2)),cast(POS_UNITS as decimal),POS_UOM from src_vw where ITEM_CONSUMER_UPC='0021727620000' and STORE='00423'
# MAGIC union all select 'etl',lpad(DIV,3,'0'),lpad(STORE,5,'0'),lpad(ITEM_CONSUMER_UPC,13,'0'),POS_DATE,cast(POS_NET_DOL_AMOUNT as decimal (8,3)),cast(POS_UOM_QTY as decimal (4,2)),cast(POS_UNITS as decimal) ,POS_UOM from gld_vw where  lpad(ITEM_CONSUMER_UPC,13,'0')='0021727620000' and lpad(STORE,5,'0')='00423'
