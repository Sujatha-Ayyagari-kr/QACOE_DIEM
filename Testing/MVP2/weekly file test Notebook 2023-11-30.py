...
# dfg= spark.read.parquet("/mnt/data/manonmani.varadharajan@kroger.com/sales/gold/part-00000-a7c494f9-2566-4610-bb4e-674f850f9da6-c000.snappy.parquet")
# dfg.createOrReplaceTempView("gld_vw")

# COMMAND ----------

dfg= spark.read.csv("/mnt/data/manonmani.varadharajan@kroger.com/weekly_sales/101_Weekly_Sales.708.20230917174432.csv",header=True)
dfg.createOrReplaceTempView("gld_vw")
...
restatesql2="select UPC_TYPE,DIV,STORE,ITEM_CONSUMER_UPC,case when index=0 then to_date(POS_DATE,'MM/dd/yyyy') else date_sub(next_day(to_date(POS_DATE,'MM/dd/yyyy'),'SAT'),0) end as POS_DATE,POS_NET_DOL_AMOUNT,POS_UOM_QTY,POS_UNITS,POS_UOM,ITEM_SRC_DIV, ITEM_SRC_LOC from vw_restate"
...
# MAGIC %sql 
# MAGIC select DIV,STORE,ITEM_CONSUMER_UPC,cast(POS_NET_DOL_AMOUNT as decimal (8,3)),cast(POS_UOM_QTY as decimal (4,2)),cast(POS_UNITS as decimal),POS_UOM from src_vw 
# MAGIC minus select lpad(DIV,3,'0'),lpad(STORE,5,'0'),lpad(ITEM_CONSUMER_UPC,13,'0'),cast(POS_NET_DOL_AMOUNT as decimal (8,3)),cast(POS_UOM_QTY as decimal (4,2)),cast(POS_UNITS as decimal),POS_UOM from gld_vw
# MAGIC
...
# MAGIC union all select 'gld' as tabnm,lpad(DIV,3,'0'),lpad(STORE,5,'0'),lpad(ITEM_CONSUMER_UPC,13,'0'),cast(POS_NET_DOL_AMOUNT as decimal (8,3)),cast(POS_UOM_QTY as decimal (4,2)),cast(POS_UNITS as decimal),POS_UOM,ITEM_SRC_DIV from gld_vw where ITEM_CONSUMER_UPC like '%4032%'
