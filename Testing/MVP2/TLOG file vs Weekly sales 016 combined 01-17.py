...
dfo= spark.read.load("/mnt/data/manonmani.varadharajan@kroger.com/weekly_sales/tlo0047_fcb_non_ace_movement_OCADO_016.dat.20240107085327",format="csv",header="false")
dfo.createOrReplaceTempView('tlogfileo')
...
dfe2=spark.sql("select DIV,STORE,ITEM_CONSUMER_UPC,sum(POS_NET_DOL_AMOUNT) as POS_NET_DOL_AMOUNT,sum(POS_UOM_QTY) as POS_UOM_QTY,sum(POS_UNITS) as POS_UNITS from weeklyfile where  ITEM_CONSUMER_UPC<>'0000000000000' and (POS_NET_DOL_AMOUNT <>0 or POS_UOM_QTY<>0 or  POS_UNITS<>0) group by DIV,STORE,ITEM_CONSUMER_UPC")
dfe2.createOrReplaceTempView("weeklyfile2")

# COMMAND ----------

dft2=spark.sql("select substr(_c0,1,3) as KMA_NO,substr(_c0,4,5) as STO_NO,  substr(_c0,10,10) as GNC_EFF_DT,substr(_c0,20,13) as SCN_UPC_NO,  substr(_c0,34,7) as SCN_MOV_QY, substr(_c0,42,9) as WGT_QY,  substr(_c0,52,9) as SCN_RTL_AM  from tlogfile where substr(_c0,9,1)='M' union all select substr(_c0,1,3) as KMA_NO,substr(_c0,4,5) as STO_NO,  substr(_c0,10,10) as GNC_EFF_DT,substr(_c0,20,13) as SCN_UPC_NO,  substr(_c0,34,7) as SCN_MOV_QY, substr(_c0,42,9) as WGT_QY,  substr(_c0,52,9) as SCN_RTL_AM  from tlogfileo where substr(_c0,9,1)='M'")
dft2.createOrReplaceTempView("tlogfile2")
...
# MAGIC %sql select t.SCN_UPC_NO as ITEM_CONSUMER_UPC,sum(t.SCN_RTL_AM)/100, round(sum(replace(POS_NET_DOL_AMOUNT,'-','')),2) ,sum(t.SCN_RTL_AM)/100-round(sum(replace(POS_NET_DOL_AMOUNT,'-','')),2) as difference from tlogfile2  t
# MAGIC inner join  weeklyfile2 w on t.SCN_UPC_NO = w.ITEM_CONSUMER_UPC 
# MAGIC group by SCN_UPC_NO,ITEM_CONSUMER_UPC
# MAGIC order by difference

# COMMAND ----------

# MAGIC %sql select t.SCN_UPC_NO as ITEM_CONSUMER_UPC,sum(t.SCN_RTL_AM)/100 as tlog_amount, round(sum(replace(POS_NET_DOL_AMOUNT,'-','')),2) as DIEM_amount ,sum(t.SCN_RTL_AM)/100-round(sum(replace(POS_NET_DOL_AMOUNT,'-','')),2) as difference from tlogfile2  t
# MAGIC inner join  weeklyfile2 w on t.SCN_UPC_NO = w.ITEM_CONSUMER_UPC 
# MAGIC group by SCN_UPC_NO,ITEM_CONSUMER_UPC
# MAGIC order by difference desc

# COMMAND ----------

# MAGIC %sql select t.SCN_UPC_NO as ITEM_CONSUMER_UPC,sum(cast(t.SCN_MOV_QY as integer)),sum(t.WGT_QY)/100,sum(t.SCN_RTL_AM)/100, sum(cast(replace(POS_UNITS,'-','') as integer)),round(sum(replace(POS_UOM_QTY,'-','')),2),round(sum(replace(POS_NET_DOL_AMOUNT,'-','')),2) ,sum(t.SCN_RTL_AM)/100-round(sum(replace(POS_NET_DOL_AMOUNT,'-','')),2) as difference from tlogfile2  t
# MAGIC inner join  weeklyfile2 w on t.SCN_UPC_NO = w.ITEM_CONSUMER_UPC 
# MAGIC  
# MAGIC  group by SCN_UPC_NO,ITEM_CONSUMER_UPC
# MAGIC order by ITEM_CONSUMER_UPC
...
# MAGIC (
# MAGIC select 'tlog' as tab,SCN_UPC_NO as ITEM_CONSUMER_UPC  from tlogfile2 where (SCN_RTL_AM <>0 or WGT_QY<>0 or  SCN_MOV_QY<>0)  group by SCN_UPC_NO
# MAGIC union all
# MAGIC select 'weekly' as tab,ITEM_CONSUMER_UPC from weeklyfile2 where (POS_NET_DOL_AMOUNT <>0 or POS_UOM_QTY<>0 or  POS_UNITS<>0)  group by ITEM_CONSUMER_UPC
...
# MAGIC %sql select 'tlog', STO_NO as STORE,SCN_UPC_NO as ITEM_CONSUMER_UPC ,cast(SCN_MOV_QY as INTEGER) as  POS_UNITS,WGT_QY/100 as POS_UOM_QTY,SCN_RTL_AM/100 as POS_NET_DOL_AMOUNT  from tlogfile2 where SCN_UPC_NO='0894047000062' and STO_NO='00522'
# MAGIC union all
# MAGIC select 'weekly',STORE,ITEM_CONSUMER_UPC,cast(POS_UNITS as integer),round(POS_UOM_QTY,2),replace(round(POS_NET_DOL_AMOUNT,2),'-','') from weeklyfile2  where ITEM_CONSUMER_UPC='0894047000062' and store='00522'
# MAGIC ORDER BY STORE

# COMMAND ----------

# MAGIC %sql select *  from weeklyfile where ITEM_CONSUMER_UPC='0894047000062' and STORE='00522'
