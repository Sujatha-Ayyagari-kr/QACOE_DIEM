...
dfg= spark.read.csv("/mnt/data/manonmani.varadharajan@kroger.com/weekly_sales/101_Weekly_Sales.014.20231217055446.csv",header=True)
...
sfile1=spark.sql("select path,name from vw_rawFileList where name like '101_Daily_Sales.014.%'")
...
# MAGIC %sql select * from src_vw where ITEM_CONSUMER_UPC like '%0001111062172%' and store ='00336'

# COMMAND ----------

# MAGIC %sql select * from gld_vw where ITEM_CONSUMER_UPC like '%0001111062172%' and store ='00336'

# COMMAND ----------

# MAGIC %sql select 'raw' as tabnm,DIV,STORE,ITEM_CONSUMER_UPC,cast(POS_NET_DOL_AMOUNT as decimal (8,3)),cast(POS_UOM_QTY as decimal (4,2)),cast(POS_UNITS as decimal),POS_UOM,ITEM_SRC_DIV from src_vw where ITEM_CONSUMER_UPC like '%0001111080288%'
# MAGIC union all select 'gld' as tabnm,DIV,STORE,ITEM_CONSUMER_UPC,cast(POS_NET_DOL_AMOUNT as decimal (8,3)),cast(POS_UOM_QTY as decimal (4,2)),cast(POS_UNITS as decimal),POS_UOM,ITEM_SRC_DIV from gld_vw where ITEM_CONSUMER_UPC like '%0001111080288%'
