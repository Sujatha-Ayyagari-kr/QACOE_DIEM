...
# MAGIC %sql select UPC_TYPE,DIV,STORE,ITEM_CONSUMER_UPC,POS_DATE,POS_NET_DOL_AMOUNT,POS_UOM_QTY,POS_UNITS,POS_UOM,ITEM_SRC_DIV,ITEM_SRC_LOC,MODALITY,FULFILLMENT,index, row_number() over(partition by DIV,STORE,ITEM_CONSUMER_UPC,POS_DATE,ITEM_SRC_DIV,MODALITY,FULFILLMENT order by index desc) rn from vw_union

# COMMAND ----------

# MAGIC %sql select * from (select UPC_TYPE,DIV,STORE,ITEM_CONSUMER_UPC,POS_DATE,POS_NET_DOL_AMOUNT,POS_UOM_QTY,POS_UNITS,POS_UOM,ITEM_SRC_DIV,ITEM_SRC_LOC,MODALITY,FULFILLMENT,index, row_number() over(partition by DIV,STORE,ITEM_CONSUMER_UPC,POS_DATE,ITEM_SRC_DIV,MODALITY,FULFILLMENT order by index desc) rn from vw_union) where rn=1
...
# MAGIC %sql select UPC_TYPE,DIV,STORE,ITEM_CONSUMER_UPC,date_sub(next_day(to_date(POS_DATE,'MM/dd/yyyy'),'SAT'),0),POS_NET_DOL_AMOUNT,POS_UOM_QTY,POS_UNITS,POS_UOM,ITEM_SRC_DIV from vw_restate
