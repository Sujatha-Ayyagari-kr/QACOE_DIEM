...
df= spark.read.load("/mnt/data/manonmani.varadharajan@kroger.com/weekly_sales/tlo0047_fcb_non_ace_movement_OCADO_016.dat.20240107085327",format="csv",header="false")
...
# MAGIC select min(tab) as tab,ITEM_CONSUMER_UPC ,POS_NET_DOL_AMOUNT  from
# MAGIC (
# MAGIC select 'tlog' as tab,SCN_UPC_NO as ITEM_CONSUMER_UPC ,sum(SCN_RTL_AM)/100 as POS_NET_DOL_AMOUNT  from tlogfile2 where (SCN_RTL_AM <>0 or WGT_QY<>0 or  SCN_MOV_QY<>0) and STO_NO='00522' group by SCN_UPC_NO
# MAGIC union all
# MAGIC select 'weekly' as tab,ITEM_CONSUMER_UPC,round(sum(replace(POS_NET_DOL_AMOUNT,'-','')),2) from weeklyfile2 where (POS_NET_DOL_AMOUNT <>0 or POS_UOM_QTY<>0 or  POS_UNITS<>0) and STORE='00522' group by ITEM_CONSUMER_UPC
# MAGIC ) group by ITEM_CONSUMER_UPC ,POS_NET_DOL_AMOUNT
# MAGIC having count(1)<>2
# MAGIC order by ITEM_CONSUMER_UPC
...
# MAGIC %sql 
# MAGIC
# MAGIC select 'tlog' as tab,sum(SCN_RTL_AM)/100 as POS_NET_DOL_AMOUNT   from tlogfile2 where (SCN_RTL_AM <>0 or WGT_QY<>0 or  SCN_MOV_QY<>0) and SCN_UPC_NO='0001111091489' union all
# MAGIC select 'weekly' as tab,round(sum(replace(POS_NET_DOL_AMOUNT,'-','')),2) from weeklyfile2 where (POS_NET_DOL_AMOUNT <>0 or POS_UOM_QTY<>0 or  POS_UNITS<>0) and ITEM_CONSUMER_UPC='0001111091489'
# MAGIC
# MAGIC

# COMMAND ----------

# MAGIC %sql select 'tlog', STO_NO as STORE,SCN_UPC_NO as ITEM_CONSUMER_UPC ,cast(SCN_MOV_QY as INTEGER) as  POS_UNITS,WGT_QY/100 as POS_UOM_QTY,SCN_RTL_AM/100 as POS_NET_DOL_AMOUNT  from tlogfile2 where SCN_UPC_NO='0001111091489' 
# MAGIC union all
# MAGIC select 'weekly',STORE,ITEM_CONSUMER_UPC,cast(POS_UNITS as integer),round(POS_UOM_QTY,2),replace(round(POS_NET_DOL_AMOUNT,2),'-','') from weeklyfile2  where ITEM_CONSUMER_UPC='0001111091489' 
# MAGIC ORDER BY STORE

# COMMAND ----------

# MAGIC %sql select *  from tlogfile2 where SCN_UPC_NO='0001111091489'

# COMMAND ----------

# MAGIC %sql select *  from weeklyfile2 where ITEM_CONSUMER_UPC='0001111091489'
...
# MAGIC %sql select 'tlog', STO_NO as STORE,SCN_UPC_NO as ITEM_CONSUMER_UPC ,cast(SCN_MOV_QY as INTEGER) as  POS_UNITS,WGT_QY/100 as POS_UOM_QTY,SCN_RTL_AM/100 as POS_NET_DOL_AMOUNT  from tlogfile2 where SCN_UPC_NO='0010000009272' and STO_NO='00522'
# MAGIC union all
# MAGIC select 'weekly',STORE,ITEM_CONSUMER_UPC,cast(POS_UNITS as integer),round(POS_UOM_QTY,2),replace(round(POS_NET_DOL_AMOUNT,2),'-','') from weeklyfile2  where ITEM_CONSUMER_UPC='0010000009272' and store='00522'
# MAGIC ORDER BY STORE

# COMMAND ----------

# MAGIC %sql select *  from tlogfile2 where SCN_UPC_NO='0010000009272' and STO_NO='00522'
